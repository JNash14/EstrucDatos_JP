    ...
    internal class GrafoPrim
    {
        ...
        public void prim(ListBox list)
        {
            ...
            while (verticesVisitados.Count < numVertices)
            {
                ...
                foreach (int verticesArbol in verticesVisitados)
                {
                    for (int i = 0; i < numVertices; i++)
                    {
                        if (!verticesVisitados.Contains(i) & matrizCosto[verticesArbol, i] != 0)
                        {
                            int costoArista = matrizCosto[verticesArbol, i];
                            if (costoArista < costoMinimo)
                            {
                                aristaMinima = new int[] { verticesArbol, i };
            ...
            list.Items.Add("Costo totao: " + costoTotal);
        }
    }
}
